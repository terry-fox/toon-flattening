...
### Unit Tests
- **Anvil Detection**:
  - `testFallingAnvilDetected()` - FallingBlockEntity above player returns entity
  - `testFallingAnvilPriority()` - Falling anvil preferred over placed block
  - `testPlacedAnvilDetected()` - Placed anvil block above player returns position
  - `testNoAnvilDetected()` - No anvil returns null
  - `testAnvilBelowPlayerIgnored()` - Anvil at feet returns null

- **Anvil Stack Counting** (NEW):
  - `testSingleAnvilCount()` - One anvil returns count = 1
  - `testThreeAnvilStackCount()` - Three stacked anvils returns count = 3
  - `testMaxStackCountCapped()` - More than 5 anvils capped at 5
  - `testNonContiguousStackBreaks()` - Air gap stops counting upward

- **Replacement Detection** (NEW):
  - `testReplacementDetected()` - hasContactingAnvil=false → isReplacement=true
  - `testStackingDetected()` - hasContactingAnvil=true → isReplacement=false
  - `testContactFlagUpdated()` - lostContact() sets hasContactingAnvil=false

- **Floor Calculation**:
  - `testFloorOnSolidBlock()` - Player on stone → floor Y = block top
  - `testFloorThroughNonSolid()` - Player on glass pane → floor Y = top of collision shape
  - `testFloorMaxDepth()` - 10 block void → floor Y = player.getY() - 10
  - `testFloorIgnoresNonCollidable()` - Barrier/structure void skipped in raycast

- **Trigger Registry**:
  - `testCustomTriggerPriority()` - Higher priority trigger checked first
  - `testCustomTriggerOverridesAnvil()` - Custom trigger blocks default anvil detection

### Integration Tests
- **Cross-Module**:
  - `testAnvilContactTriggersCompression()` - Falling anvil → core.beginCompression() with anvilCount
  - `testAnvilRemovalTriggersRecovery()` - Anvil removed → core.lostContact()
  - `testReflattenInFullyFlattened()` - Anvil on FullyFlattened → core.applyReflatten() with isReplacement
  - `testRecoveringReflatten()` - Anvil on Recovering → core.beginCompression() (NEW)
  - `testAnvilStackPassedToCore()` - 3-anvil stack → beginCompression(..., anvilCount=3) (NEW)

- **Event Handling**:
  - `testVanillaDamageCancelled()` - Anvil damage event cancelled during ProgressiveFlattening
  - `testDamageAllowedWhenNormal()` - Anvil damage event passes through when Normal phase
